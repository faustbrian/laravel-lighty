...
expect()->extend('toMatchSnapshot', function (): void {
    assertMatchesSnapshot($this->value);
});
...
{
    assertMatchesSnapshot((new DocumentParser())->parse($code)->toArray());
}
...
function assertMatchesShikiSnapshot(string $code): void
{
    ...
    assertMatchesSnapshot(Lighty::highlight($document));
}
